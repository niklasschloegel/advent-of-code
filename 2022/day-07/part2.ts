...
function solve(data: string): string {
  ...
  debug(dirSizes);
  const availableSpace = diskSize - dirSizes["/"];
  const neededDeletions = Object.values(dirSizes).filter((x) =>
    availableSpace + x >= neededUnusedSpace
  )
    .sort((a, b) => a - b);
  return neededDeletions[0].toString();
}
// --------------------------- END SOLUTION CODE ---------------------------
